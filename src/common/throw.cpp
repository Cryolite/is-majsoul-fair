...
#if defined(IS_MAJSOUL_FAIR_WITH_COVERAGE)

extern "C" void __gcov_flush();

#endif // defined(IS_MAJSOUL_FAIR_WITH_COVERAGE)
...
try {
...
#else // defined(IS_MAJSOUL_FAIR_WITH_COVERAGE)
      std::abort();
#endif // defined(IS_MAJSOUL_FAIR_WITH_COVERAGE)
    }
  ...
  try {
    ...
    std::cerr << "`std::terminate' is called after throwing an instance of `"
              << IsMajsoulFair::getTypeName(e) << "'.\n";
    printErrorMessage(e);
    if (Stacktrace const * const p = boost::get_error_info<IsMajsoulFair::StackTraceErrorInfo>(e)) {
      if (p->size() != 0) {
        std::cerr << "Backtrace:\n" << *p;
      }
    }
  ...
  while (!nested_exceptions.empty()) {
    try {
      ...
      std::cerr << "A nesting exception of type `" << IsMajsoulFair::getTypeName(e) << "'.\n";
      printErrorMessage(e);
    }
    catch (std::exception const &e) {
      std::cerr << "A nesting exception of type `" << IsMajsoulFair::getTypeName(e) << "'.\n";
